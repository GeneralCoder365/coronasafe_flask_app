...
def get_state_and_country_covid_cases(country, state):
    ...
    dtypes = {'date': 'str', 'location_key': 'str', 'new_confirmed': 'float64'}
    ...
    df_country = df[df.location_key == country_key].compute() # ! converts to pandas dataframe
    ...
    # print(country_last_date)
    country_last_date_date = (datetime.strptime(str(country_last_date), '%Y-%m-%d')).date()
    ...
    df_state = df[df.location_key == state_key].compute() # converts to pandas dataframe
